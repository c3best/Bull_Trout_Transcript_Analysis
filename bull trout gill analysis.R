#CBest 2023
#Bull Trout Gill Analysis - Transcriptional Changes due to Acclimation Temperature
...
gill_data_clean_wells <- read_excel("gill_data_qc_omit.xlsx")
...
gill_data_clean_wells <- gill_data_clean_wells %>%
...
gill_data_clean_wells_wide <- gill_data_clean_wells %>% 
...
gill_for_analysis <- as.data.frame(gill_data_clean_wells_wide)  

#Import efficiency list as a data frame
gill_efficiency_list <- read.csv("gill_linreg_efficiencies.csv")
...
qs_gill=cq2counts(data=gill_for_analysis, effic=gill_efficiency_list,
                  genecols=c(3:50),condcols=c(1:2), Cq1=37)

#Set the reference treatment to 9 degC (used for plotting model in 'relative' mode)
qs_gill$treatment=relevel(qs_gill$treatment,ref="9deg")
...
naive_gill=mcmc.qpcr(data=qs_gill, fixed="treatment")
...
naive_gill_abs <- HPDsummary(model=naive_gill,data=qs_gill)

#$summary: summary Table containing calculated abundances, their SD and 95% credible limits
naive_gill_abs_table <- naive_gill_abs$summary

#Summary Plot
summaryPlot(naive_gill_abs_table, "treatment", facet = "gene", type="line",
...
naive_gill_abs$geneWise
cat(capture.output(print(naive_gill_abs$geneWise),file="naive_gill_abs_geneWise_test.txt"))

#CORRECTION FOR MULTIPLE COMPARISONS
#Benjamini-Hotchberg method
naive_gill_abs_adj = padj.hpdsummary(naive_gill_abs, controls = NULL, method="BH")

#table/plot will be same as without adjustment - only changes p-values
naive_gill_abs_adj_table <- naive_gill_abs_adj$summary

summaryPlot(naive_gill_abs_adj_table, "treatment", facet = "gene", type="line",
            x.order=c("6deg", "9deg", "12deg","15deg", "18deg","21deg"))+
  theme(legend.position="none") +
  theme(axis.text.x = element_text(angle=90, vjust=0)) +
  facet_wrap(.~gene, scales="free_y") #can omit this to see everything on same scale

naive_gill_abs_adj$geneWise
cat(capture.output(print(naive_gill_abs_adj$geneWise),file="naive_gill_abs_adj_geneWise_test.txt"))

#Check genewise p-values - nothing significant for rpl13a, rpl7, rps9
#Check plots - no global trends, reference genes are stable/no trends
...
informed_gill_model=mcmc.qpcr(data=qs_gill, fixed="treatment", controls = c("SALRPL13A","SALRPL7","SALRPS9"))
options(max.print=2000)
summary(informed_gill_model)
...
#Use summary to check model convergence
  ...
  #Look at eff.samp column - expected (nitt-burnin)/thin = 1000
    #nitt - number of iterations, default 13000
    #burnin - number of initial iterations to discard, default 3000
    #thin - thinning interval, number of iterations beween when parameters are sampled, default 10
  #If eff.samp is much lower than 1000, then try nitt=110000, thin=100, burnin=10000
#OK
...
#You can also check model convergence using these plots
  #Should be random noise around a mean value, no waves/trends etc.
  #Plot function below will generate many plots!
  plot(informed_gill_model)

####INFORMED/ABSOLUTE####
informed_gill_abs <- HPDsummary(model=informed_gill_model, data=qs_gill)

informed_gill_abs_table <- informed_gill_abs$summary

summaryPlot(informed_gill_abs_table, xgroup="treatment", facet="gene", type="line",
            x.order=c("6deg","9deg","12deg","15deg","18deg","21deg"))+
  theme(legend.position="none")+
  theme(axis.text.x = element_text(angle=90, vjust=0))+
  facet_wrap(.~gene, scales="free_y") #can omit this to see everything on same scale

informed_gill_abs$geneWise
cat(capture.output(print(informed_gill_abs$geneWise),file="informed_gill_abs_geneWise_test.txt"))

#CORRECTION FOR MULTIPLE COMPARISONS
#Benjamini-Hotchberg method
informed_gill_abs_adj = padj.hpdsummary(informed_gill_abs, controls = NULL, method="BH")

#table/plot will be same without adjustment - only changes p-values
informed_gill_abs_adj_table <- informed_gill_abs_adj$summary

summaryPlot(informed_gill_abs_adj_table, "treatment", facet = "gene", type="line",
            x.order=c("6deg", "9deg", "12deg","15deg", "18deg","21deg"))+
  theme(legend.position="none") +
  theme(axis.text.x = element_text(angle=90, vjust=0)) +
  facet_wrap(.~gene, scales="free_y") #can omit this to see everything on same scale

informed_gill_abs_adj$geneWise
cat(capture.output(print(informed_gill_abs_adj$geneWise),file="informed_gill_abs_adj_geneWise_test.txt"))
...
DIAG_informed_gill_model=mcmc.qpcr(data=qs_gill, fixed="treatment", controls = c("SALRPL13A","SALRPL7","SALRPS9"), pr=T, pl=T)
...
diagnostic.mcmc(
  model=DIAG_informed_gill_model,
  col="grey50",
  cex=0.8
)
...
informed_gill_abs_table

#Convert to tibble and format to facilitate plotting
informed_gill_abs_tibble <- as_tibble(informed_gill_abs_table) %>%
...
gene_function_key
informed_gill_abs_tibble_SYMBOLS <- left_join(informed_gill_abs_tibble,gene_function_key,by="gene")

#LINE GRAPHS:

#Line graph (one gene - don't forget to change TITLE, and y-axis if needed)
ggplot(subset(informed_gill_abs_tibble_SYMBOLS, Symbol %in% "cpt1a"), aes(x=temp, y=mean, colour=temp)) +
  ...
  theme_classic()+
  theme(legend.position="none")+
  labs(y=expression(log[2](abundance)), x="Acclimation Temp. (°C)", title="GILL-CPT1A")+
  ylim(8.5,14)+
...
ggplot(subset(informed_gill_abs_tibble_SYMBOLS, !(Symbol %in% c("ef1a","rpl13a","rpl7","rps9"))), aes(x=temp, y=mean, colour=temp)) +
  scale_colour_manual(values = c("#053061","#4393C3","#F4A582","#D6604D","#B2182B","#67001F"))+
  geom_errorbar(aes(ymin=lower, ymax=upper), width = 0.5)+
  geom_point(position = position_dodge(), stat="identity",size=3) +
  theme_classic()+
  theme(legend.position="none")+
  labs(y=expression(log[2](abundance)), x="Acclimation Temp. (°C)", title = "Gill")+
  theme(text = element_text(size=10),
        axis.title.y = element_text(margin=margin(0,5,0,0)),
        axis.title.x = element_text(margin=margin(5,0,0,0)),
        axis.text = element_text(colour="black"))+
...
#Use gill_data_clean (just has means of replicates)
gill_data_clean <- read_excel("gill_data_clean.xlsx")

#Ct of target gene minus geomean of reference Cts
#Select just the reference genes, pivot wide, and calculate geometric mean
gill_geomean_refs <- gill_data_clean %>% select(Gene.Name, Sample.Name, Mean.Crt) %>%
...
gill_dCrt <- left_join(gill_data_clean,gill_geomean_refs,by="Sample.Name") %>%
  rowwise() %>%
...
gill_dCrt_wide_for_pca <- gill_dCrt %>% 
...
gill_dCrt_wide_for_pca
gill_dCrt_wide_for_pca_nosamp <- mutate(gill_dCrt_wide_for_pca, Sample.Name=NULL)
gill_dCrt_wide_for_pca_nosamp
...
gill_dCrt_wide_for_pca_nosamp_ORD <- gill_dCrt_wide_for_pca_nosamp %>%
...
plot(gill.dCrt.pca, graph.type="ggplot",label="none", habillage = 1,
...
plotellipses(gill.dCrt.pca, label="none", level=0.95)

#Making nicer (and more editable) PCA using ggplot

#Extract the following to plot the data
individuals.dCrt <- rownames(gill.dCrt.pca$ind$coord)
pc1.dCrt <- gill.dCrt.pca$ind$coord[,1]
pc2.dCrt <- gill.dCrt.pca$ind$coord[,2]
treatment.dCrt <- gill_dCrt_wide_for_pca_nosamp_ORD$treatment
...
#59 individuals for gill
gill.dCrt_pca_plot_data <- tibble(Ind = individuals.dCrt, Dim1 = pc1.dCrt, Dim2 = pc2.dCrt, Treatment = treatment.dCrt)
...
gill.dCrt.pca$eig
pcent_var_pc1.dCrt <- gill.dCrt.pca$eig[1,2] %>% round(digits=2)
pcent_var_pc2.dCrt <- gill.dCrt.pca$eig[2,2] %>% round(digits=2)

#Add column Temperature with degrees symbol
gill.dCrt_pca_plot_data <- gill.dCrt_pca_plot_data %>%
  mutate(Temperature = paste0(Treatment,"°C")) %>%
  mutate(Temperature = as_factor(Temperature))

#Plot the PCA with ggplot
gill.dCrt_PCA_graph <- ggplot(data = gill.dCrt_pca_plot_data, aes(x = Dim1, y = Dim2, colour = Temperature)) +
...
gill.dCrt.loadings <- sweep(gill.dCrt.pca$var$coord,2,sqrt(gill.dCrt.pca$eig[1:ncol(gill.dCrt.pca$var$coord),1]),FUN="/")

gill.dCrt.loadings.tib <- as_tibble(gill.dCrt.loadings, rownames="Gene")
gill.dCrt.loadings.tib

gill.dCrt.loadings.tib <- left_join(gill.dCrt.loadings.tib,gene_function_key,by="Gene")

#Plot PC1 loadings just to get legend for panel
forlegend.dCrt<-ggplot(data=gill.dCrt.loadings.tib, aes(y=reorder(Symbol,Dim.1), x=Dim.1, fill=Function))+
...
#PC1 Loadings Plot
a.dCrt<-ggplot(data=gill.dCrt.loadings.tib, aes(y=reorder(Symbol,Dim.1), x=Dim.1, fill=Function))+
  scale_fill_paletteer_d("ggthemes::Superfishel_Stone")+
  geom_bar(position = position_dodge(), stat="identity")+
  theme_classic()+
  theme(axis.text.y = element_text(face = "italic"))+
...
#PC2 Loadings Plot
b.dCrt<-ggplot(data=gill.dCrt.loadings.tib, aes(y=reorder(Symbol,Dim.2), x=Dim.2, fill=Function))+
  scale_fill_paletteer_d("ggthemes::Superfishel_Stone")+
  geom_bar(position = position_dodge(), stat="identity")+
  theme_classic()+
  theme(axis.text.y = element_text(face = "italic"))+
